...
function SVGPiecesImpl({
  ...
  for (let y = 0; y < bitMatrix.length; y++) {
    for (let x = 0; x < bitMatrix.length; x++) {
      ...
      if (bitMatrix[y]?.[x] === 1) {
        const originX = x * pieceSize + pieceSize / 2;
        const originY = y * pieceSize + pieceSize / 2;
          ...
          if (
            ...
            if (bitMatrix[y]?.[x - 1] === 1 && bitMatrix[y + 1]?.[x] === 1) {
              svgPiecesMatrix.push(
                <Path rotation={-90} originX={originX} originY={originY} key={`${x}_${y}_topRight`} d={d} />
              );
            }

            if (bitMatrix[y]?.[x + 1] === 1 && bitMatrix[y - 1]?.[x] === 1) {
              svgPiecesMatrix.push(
                <Path rotation={90} originX={originX} originY={originY} key={`${x}_${y}_bottomRight`} d={d} />
              );
            }

            if (bitMatrix[y]?.[x + 1] === 1 && bitMatrix[y + 1]?.[x] === 1) {
              svgPiecesMatrix.push(
                <Path rotation={180} originX={originX} originY={originY} key={`${x}_${y}_bottomLeft`} d={d} />
              );
            }
          }
        }
  ...
  if (outerEyesOptions) {
    ...
    const origins = {
      topLeft: { x: (pieceSize * OUTER_EYE_SIZE_IN_BITS) / 2, y: (pieceSize * OUTER_EYE_SIZE_IN_BITS) / 2 },
      topRight: { x: qrSize - (pieceSize * OUTER_EYE_SIZE_IN_BITS) / 2, y: (pieceSize * OUTER_EYE_SIZE_IN_BITS) / 2 },
      bottomLeft: { x: (pieceSize * OUTER_EYE_SIZE_IN_BITS) / 2, y: qrSize - (pieceSize * OUTER_EYE_SIZE_IN_BITS) / 2 },
    };
    ...
    listPositions.forEach((position) => {
      ...
      svgPiecesMatrix.push(
        <Path
          ...
          originX={origins[position].x}
          originY={origins[position].y}
  ...
  if (innerEyesOptions) {
    const listPositions: EyePosition[] = ['topLeft', 'topRight', 'bottomLeft'];
    const origins = {
      topLeft: { x: pieceSize * 2 + (pieceSize * INNER_EYE_SIZE_IN_BITS) / 2, y: pieceSize * 2 + (pieceSize * INNER_EYE_SIZE_IN_BITS) / 2 },
      topRight: { x: qrSize - pieceSize * 2 - (pieceSize * INNER_EYE_SIZE_IN_BITS) / 2, y: pieceSize * 2 + (pieceSize * INNER_EYE_SIZE_IN_BITS) / 2 },
      bottomLeft: { x: pieceSize * 2 + (pieceSize * INNER_EYE_SIZE_IN_BITS) / 2, y: qrSize - pieceSize * 2 - (pieceSize * INNER_EYE_SIZE_IN_BITS) / 2 },
    };
